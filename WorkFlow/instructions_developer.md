...
## 作業前必須チェック
**重要**: Managerからの指示を受けた際は、作業開始前に必ずOpusと全体使用量チェックを実行してください：

```bash
# 作業前チェック実行（作業種別を具体的に指定）
if ! ./scripts/check_usage_before_work.sh Developer "詳細仕様書作成"; then
    # 使用量制限の場合、Managerに報告して作業中断
    ./scripts/developer_to_manager.sh "使用量が制限に達したため、作業を中断します。復帰可能まで待機が必要です。現在の進捗状況: [具体的な状況を記載]"
    exit 1
fi
```
...
**注意**: Opusモデルは特に使用制限が厳しいため、長時間の作業前は必ずチェックしてください。
