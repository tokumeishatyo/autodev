...
recover_system() {
    ...
    if ! check_tmux_session; then
        ...
        # 各ペインの役割を設定
        tmux send-keys -t "$TMUX_SESSION:0.0" "echo 'Manager ペイン復旧完了'" C-m
        tmux send-keys -t "$TMUX_SESSION:0.1" "echo 'Developer ペイン復旧完了'" C-m
        tmux send-keys -t "$TMUX_SESSION:0.2" "echo 'Reviewer ペイン復旧完了'" C-m
        tmux send-keys -t "$TMUX_SESSION:0.3" "echo 'CEO ペイン復旧完了'" C-m
...
perform_health_check() {
    ...
    if ! check_tmux_session; then
        ...
        check_pane "2" "Reviewer" || all_ok=false
        check_pane "3" "CEO" || all_ok=false
    fi
